...
from backend.safety.gate import decide_confirmation, defer_confirmation, defer_expired_workspace, is_expired
...
def list_confirmations(
    ...
    """待确认队列（已离开工作区的确认）：先做惰性超时迁移，再只返回队列中的 pending。"""
    defer_expired_workspace(db)
    query = db.query(Confirmation).filter(
        Confirmation.status == "pending", Confirmation.in_workspace.is_(False)
    )
...
@router.post("/{confirmation_id}/defer", response_model=ConfirmationResponse)
def defer(
    confirmation_id: int,
    db: Session = Depends(get_db),
    current_user: int | None = Depends(get_current_user),
):
    """「稍后」：把工作区确认转入待确认队列；已转/已决幂等。"""
    row = db.get(Confirmation, confirmation_id)
    if row is None:
        raise HTTPException(status_code=404, detail="确认记录不存在")
    if current_user is not None and row.task_id:
        owner = db.get(Task, row.task_id)
        if owner and owner.user_id is not None and owner.user_id != current_user:
            raise HTTPException(status_code=403, detail="无权处理该确认（任务属于其他用户）")
    if row.status != "pending" or not row.in_workspace:
        return _to_response(row)
    row = defer_confirmation(db, confirmation_id)
    from backend.safety.audit import log_audit

    log_audit(
        db,
        user_id=current_user,
        action="confirmation.defer",
