    ...
    def execute(self, action: str = "", **kwargs) -> ToolResult:
        if action == "send":
            return self.send(
                ...
                mail_auth_code=kwargs.get("mail_auth_code"),
            )
        if action == "read":
            return self.read(limit=int(kwargs.get("limit", 10)))
        return ToolResult(ok=False, message=f"不支持的邮件动作：{action}")

    def send(self, subject: str, to: str, body: str, mail_address: str | None = None, mail_auth_code: str | None = None) -> ToolResult:
        """发邮件；可指定发件邮箱+授权码（用户绑定），否则用系统默认。"""
        addr = (mail_address or settings.qq_mail_address).strip()
        code = (mail_auth_code or settings.qq_mail_auth_code).strip()
        if not addr or not code:
            return ToolResult(ok=False, message="QQ 邮箱未配置（用户未绑定且系统未配置）")
        if not to or not subject:
            return ToolResult(ok=False, message="收件人和主题不能为空")
        msg = EmailMessage()
        msg["From"] = formataddr(("办公自动化 Agent", addr))
        msg["To"] = to
        msg["Subject"] = subject
